   ...
   public function createOrder(Order $order)
{
    ...
    Log::info('Razorpay credentials loaded for createOrder', [
        'key' => $razorpayKey,
    ...
    if (empty($razorpayKey) || empty($razorpaySecret)) {
        return redirect()->route('orders.thankyou', $order->id)
            ->with('error', 'Payment gateway credentials are missing. Please contact support.');
    }
    ...
    try {
        ...
        return Inertia::render('RazorpayPayment', [
            ...
            'razorpay_key' => $razorpayKey, // ✅ same key used in order creation
        ...
        return redirect()->route('orders.thankyou', $order->id)
            ->with('error', 'Failed to initiate Razorpay payment: ' . $e->getMessage());
    }
}
   ...
   public function retryPayment(Order $order)
{
    if ($order->status !== 'pending') {
        Log::warning('Invalid payment retry request', [
            'order_id' => $order->id,
            'status' => $order->status
        ]);
        return redirect()->route('orders.thankyou', $order->id)
            ->with('error', 'Invalid payment retry request.');
    }
    ...
    Log::info('Razorpay credentials loaded for retry', [
        'key' => $razorpayKey,
        'secret_present' => !empty($razorpaySecret)
    ]);

    // ✅ If credentials are missing, return error early
    if (empty($razorpayKey) || empty($razorpaySecret)) {
        return redirect()->route('orders.thankyou', $order->id)
            ->with('error', 'Payment gateway credentials are missing. Please contact support.');
    }
    ...
    try {
        ...
        Log::info('Razorpay retry order created', [
            'razorpay_order_id' => $razorpayOrder->id
        ]);
        ...
        Log::error('Razorpay retry order creation failed', [
            'error' => $e->getMessage(),
            'order_id' => $order->id
        ]);
        return redirect()->route('orders.thankyou', $order->id)
            ->with('error', 'Failed to retry Razorpay payment: ' . $e->getMessage());
    }
}
